...
export class HubSDK {
  constructor() {
    ...
    this.gameId = null;
    this.hubOrigin =
      (typeof window !== "undefined" && window.HUB_CONFIG?.origin) ||
  ...
  init(config = {}) {
    ...
    logger.info(
      LogCategory.APP,
      `[HubSDK] Master Bridge Initialized for ${this.gameId}`,
    );
  }
  ...
  onSettings(callback) {
    this.callbacks.onSettings.push(callback);
  }
  onMute(callback) {
    this.callbacks.onMute.push(callback);
  }
  onUnmute(callback) {
    this.callbacks.onUnmute.push(callback);
  }
  ...
  handleMessage(event) {
    // Security check
    if (event.origin !== this.hubOrigin && !event.origin.includes("localhost"))
      return;
  ...
  _send(type, payload) {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type, payload: payload || {} }, this.hubOrigin);
    } else {
      logger.warn(
        LogCategory.APP,
        "[HubSDK] No parent window found. Running standalone?",
      );
      // Standalone fallback: redirects if exit is called
      if (type === "GAME_EXIT") {
        const hubPath = window.location.pathname.includes("/games/")
          ? "../../index.html"
          : "./index.html";
